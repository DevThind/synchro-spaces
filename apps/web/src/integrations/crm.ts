...
export class HubSpotAdapter implements CrmAdapter {
  ...
  async submit(lead: LeadPayload, requestId: string) {
    const response = await fetch("https://api.hubapi.com/crm/v3/objects/contacts", { method: "POST", headers: { Authorization: `Bearer ${this.accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ properties: { email: lead.email || undefined, firstname: lead.name, phone: lead.phone || undefined, project_audience: lead.audience, project_location: lead.location, services_of_interest: lead.servicesOfInterest.join(";"), project_stage: lead.stage, message: lead.message, website_request_id: requestId } }), cache: "no-store" });
...
export class GoHighLevelAdapter implements CrmAdapter {
  ...
  async submit(lead: LeadPayload, requestId: string) {
    const response = await fetch("https://services.leadconnectorhq.com/contacts/", { method: "POST", headers: { Authorization: `Bearer ${this.apiKey}`, Version: "2021-07-28", "Content-Type": "application/json" }, body: JSON.stringify({ name: lead.name, email: lead.email || undefined, phone: lead.phone || undefined, locationId: this.locationId, source: "Website consultation", tags: lead.audience ? [lead.audience] : [], customFields: [{ key: "project_location", field_value: lead.location }, { key: "services", field_value: lead.servicesOfInterest.join(", ") }, { key: "request_id", field_value: requestId }] }), cache: "no-store" });
    if (!response.ok) throw new Error("CRM_SUBMISSION_FAILED");
