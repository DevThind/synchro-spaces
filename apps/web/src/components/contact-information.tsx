...
export function ContactInformation() {
  ...
  return <aside className="aside-card"><h2>Connect with {siteConfig.companyName}</h2><ul className="aside-list">
    ...
    {hasVerifiedEmail ? <li><Mail size={17} aria-hidden="true" /> <strong>Email</strong><br /><a href={`mailto:${siteConfig.email}`} aria-label={`Email ${siteConfig.companyName} at ${siteConfig.email}`} data-analytics-event="email_click">{siteConfig.email}</a></li> : null}
